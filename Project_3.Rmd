...
#Simple Tune of Logistic Regression Model
set.seed(123)
logit_model_tuned <- train(as.factor(satisfaction) ~. - Flight.Distance, data = airline_train, method = "glm", family = "binomial")
summary(logit_model_tuned)
```
<br />
<br />
...
#optimal_ctrl <- optimalCutoff(airline_test$satisfaction, prediction_ctrl)
#binary_predictions_ctrl <- ifelse(prediction_ctrl > optimal, 1, 0)
...
*The neural net algorithm provides a prediction of satisfaction by using nodes that model the neurons in our brain. To optimize this model, we adjusted the number of hidden nodes and the probability cutoff for a satisfied (1) prediction. We found that adding hidden nodes helped with accuracy. Going to 2 hidden nodes increased the accuracy to about 80%. However, adding any more hidden nodes than 2 proved to be too computationally demanding for the computer to execute. Thereafter, we employed the optimalCutoff function to determine that a cutoff of 33.3% was the optimal value for maximizing accuracy. At its optimal parameters, accuracy is about 80%, sensitivity is about 90%, and Kappa is 0.572. Thus, this model underperforms the logistic regression and KNN.* 
