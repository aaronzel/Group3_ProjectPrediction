...
```{r}
combinedTable1 <- data.frame(ANN = as.numeric(predicted_ANN_results), kNN = as.numeric(satifaction_test_pred_knn)-1, logistic = as.numeric(binary_predictions), SVM = as.numeric(predictions_svm))

combinedTable1$sum <- rowSums(combinedTable1)
combinedTable1$combinedPredictions <- ifelse(combinedTable1$sum >= 2, 1, 0) 

caret::confusionMatrix(combinedTable1$combinedPredictions, airline_test$satisfaction)
```
