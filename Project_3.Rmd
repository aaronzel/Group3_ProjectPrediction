...
ggplot(airline_train,
  ...
  labs(y = "Number Satisfied", x = "Inflight Wifi Service Rating", fill = "Satisfaction", title = "Inflight Wifi Service Rating vs Number Satisfied") + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 90))
```

*The chart above displays the distribution between the Inflight Wifi Service Rating and the corresponding number of people who were satisfied/unsatisfied. As expected, as the rating for inflight wifi service increases, so does the number of people who are satisfied. Interestingly, the number of people who are neutral or dissatisfied is very similar if the rating is 2 or 3, which is in line with the mean inflight wifi service rating as calculated above. Further, it is a surprise that all of the customers who gave a 0 rating for inflight wifi service were all satisfied, according to the chart. If the airline can increase this average rating amongst their customers, the number of satisfactory surveys will increase.*
...
ggplot(airline_train,
       aes(x = Ease.of.Online.booking,
           ...
           labels = c("Neutral or Dissatisfied", "Satisfied")))) +
  geom_bar(position = "dodge") +
  labs(y = "Number Satisfied", x = "Ease of Online Booking Rating", fill = "satisfaction", title = "Ease of Online Booking vs Number Satisfied")+ theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 90))
```
...
ggplot(airline_train,
       aes(x = Gate.location,
           ...
           labels = c("Neutral or Dissatisfied", "Satisfied")))) +
  geom_bar(position = "dodge")+
  labs(y = "Number Satisfied", x = "Gate Location Rating", fill = "satisfaction", title = "Gate Location Rating vs Number Satisfied") + theme(plot.title = element_text(hjust = 0.5), axis.text.x = element_text(angle = 90))
```
...
#save(airlineneuralnet1, file="airline_neural_net.RData")
load("airline_neural_net.RData")
...
```{r, warning=FALSE}
#building SVM model
#svm_model1 <- ksvm(as.factor(satisfaction) ~., data = airline_train_factorized_normalized, kernel = "vanilladot", scaled = TRUE)
...
*The 1st combined model is based on a simple voting system. If 4 or more models predict satisfied (1), then the combined model predicts satisfied. This leads to an accuracy of 90.5%, sensitivity of 98%, and Kappa of 0.803, making it a relatively good model. Next, we will look at an ensemble method.*
<br />
<br />
...
```
*The 2nd combined model, the stacked model, improves on the combined model with an accuracy at 96.3%, kappa value at 0.925, indicating near perfect agreement, and sensitivity at 97.9%.*
